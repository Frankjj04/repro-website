...
-- Payments, added September 2026. Stripe tells the site when a family pays
-- (api/stripe.js); every payment is kept here, matched to a player or not, so
-- the coach can see one that could not be tied to anybody.
-- paid_at / paid_amount on the player are the first matched payment.
-- details_sent_at: when the venue-and-time email went out after paying.
ALTER TABLE applicants ADD COLUMN IF NOT EXISTS paid_at TIMESTAMPTZ;
ALTER TABLE applicants ADD COLUMN IF NOT EXISTS paid_amount TEXT NOT NULL DEFAULT '';
ALTER TABLE applicants ADD COLUMN IF NOT EXISTS details_sent_at TIMESTAMPTZ;
ALTER TABLE applicants ADD COLUMN IF NOT EXISTS details_to TEXT NOT NULL DEFAULT '';

CREATE TABLE IF NOT EXISTS payments (
  id             BIGSERIAL PRIMARY KEY,
  -- Stripe's Checkout Session id. Unique, so a message Stripe sends twice is
  -- recorded once.
...
-- Delivery of every invitation and details email, added September 2026. One
-- row per email per address, keyed by Resend's id; Resend reports back through
-- /api/resend. status: sent → delayed → delivered, or bounced / complained.
CREATE TABLE IF NOT EXISTS emails (
  resend_id      TEXT PRIMARY KEY,
  applicant_id   BIGINT NOT NULL,
  kind           TEXT NOT NULL CHECK (kind IN ('invite', 'details')),
  recipient      TEXT NOT NULL,
  status         TEXT NOT NULL DEFAULT 'sent'
                 CHECK (status IN ('sent', 'delayed', 'delivered', 'bounced', 'complained')),
  status_detail  TEXT NOT NULL DEFAULT '',
  status_at      TIMESTAMPTZ,
  created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS emails_applicant_idx ON emails (applicant_id, created_at);
