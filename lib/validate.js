...
export function validateApplication(body) {
  ...
  // Age rules per event — see js/registro-config.js. A younger player may
  // always play up; only players older than the date's range are turned away,
  // and only on the dates the coach limited.
  const born = Number(a.dob.slice(0, 4));
  if (event.rule === 'no_older' && born < event.years[0]) {
    return fail('event_age', 'That date is for players born ' + event.years[0] + ' or later.');
  }
