...
/* Social handles, which people give in every possible shape: "@nombre",
   "nombre", "instagram.com/nombre", the full https URL, sometimes with a
   trailing slash or a ?query. All of them come back as the bare handle.
   Empty is fine — both fields are optional. */
export function handle(s, host) {
  let v = String(s || '').trim();
  if (!v) return '';
  v = v.replace(/^https?:\/\//i, '').replace(/^www\./i, '');
  if (host && v.toLowerCase().startsWith(host)) v = v.slice(host.length);
  v = v.split(/[/?#]/)[0].replace(/^@+/, '').trim();
  if (!v) return '';
  // Instagram and TikTok both allow letters, digits, dots and underscores.
...
export function validateApplication(body) {
  ...
  // Optional, and asked for because scouts look players up: the coach wants
  // to be able to see them without hunting.
  a.instagram = handle(b.instagram, 'instagram.com/');
  if (a.instagram === null) return fail('instagram', 'That Instagram name does not look right.');
  a.tiktok = handle(b.tiktok, 'tiktok.com/');
  if (a.tiktok === null) return fail('tiktok', 'That TikTok name does not look right.');
  ...
  // Every minor: we keep the parent's own email. They are the ones who sign the
  // waiver, who pay, and who bring the player — the invitation has to reach an
  // adult, not only a phone the child carries.
  a.isChild = age < CHILD_AGE;
  if (a.isMinor) {
    if (!/^[^\s@]+@[^\s@]+\.[a-z]{2,}$/i.test(a.parentEmail)) {
      return fail('parent_email', "A parent or guardian's email is required for players under " + MINOR_AGE + '.');
    }
  } else {
    a.parentEmail = '';
  }

  // Under 13 the form has to be filled BY the parent, who says plainly that it
  // is them. US rules on children's data expect the parent, not the child.
  if (a.isChild && b.parentConfirm !== true) {
