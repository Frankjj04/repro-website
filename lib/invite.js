...
const pick = (v, lang) => (v && typeof v === 'object' ? (v[lang] || v.es || '') : String(v || ''));

const eventDates = (id, lang) => {
...
export function buildInvite(a, opts = {}) {
  ...
  if (log.venue) lines.push(t.where.toUpperCase(), log.venue, log.time, '');
  lines.push(t.pay_h.toUpperCase());
  lines.push(`${t.pay_amount}: ${pick(PAYMENT.amount, lang)}`);
  for (const m of PAYMENT.methods) lines.push(`${m.label}: ${m.detail}`);
  lines.push(`${t.pay_deadline}: ${pick(PAYMENT.deadline, lang)}`);
  lines.push(`${t.pay_refund}: ${pick(PAYMENT.refund, lang)}`, '');
  ...
  const p = (s) => `<p style="margin:0 0 10px;font:400 15px/1.6 Arial,sans-serif;color:#22263A;">${s}</p>`;
...
<title>${esc(t.subject(dates))}</title></head>
...
<table role="presentation" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;width:100%;background:#fff;border-radius:12px;overflow:hidden;">
  ...
  ${log.venue ? box(t.where, p(`${esc(log.venue)}<br />${esc(log.time)}`)) : ''}
  ${box(t.pay_h,
    p(`<strong>${esc(t.pay_amount)}: ${esc(pick(PAYMENT.amount, lang))}</strong>`) +
    `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:6px 0 10px;">${payRows}</table>` +
    p(`${esc(t.pay_deadline)}: ${esc(pick(PAYMENT.deadline, lang))}`) +
    p(`<span style="color:#5B6076;">${esc(t.pay_refund)}: ${esc(pick(PAYMENT.refund, lang))}</span>`),
    '#FF6B1A')}
