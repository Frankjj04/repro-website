...
export function validateApplication(body) {
  ...
  // Each event is for certain birth years — see js/registro-config.js.
  const born = Number(a.dob.slice(0, 4));
  if (event.years && (born < event.years[0] || born > event.years[1])) {
    return fail('event_age', 'That event is for players born ' + event.years[0] + '-' + event.years[1] + '.');
  }
